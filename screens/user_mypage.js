...
const SettingsScreen = () => {
  ...
  const orderlist = async () => {
    ...
    if (userInfo) { 
      try {
        const response = await axios.post('https://18.188.101.208:8090/botbuddies/orderlist', { id: userInfo[0].user_id });
  ...
  const reservaList = async () => {
    ...
    if (userInfo) { 
      try {
        const response = await axios.post('https://18.188.101.208:8090/botbuddies/reservaList', { id: userInfo[0].user_id });
  ...
  const handleReviewManagement = async () => {
    ...
    try {
      const response = await axios.post('https://18.188.101.208:8090/botbuddies/reviewModify', {  id: userInfo[0].user_id });
  ...
  const inquiryCheck = async () => {
    // 서버로부터 데이터를 받아오는 로직 구현
    try {
      const response = await axios.post('https://18.188.101.208:8090/botbuddies/inquiryCheck', {  id: userInfo[0].user_id });
