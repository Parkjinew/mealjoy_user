  ...
  console.log(store)
    ...
    const orderT = async(orderDetails) => {
      ...
      if(store.category_seq==2){
        const response = await axios.post('http://18.188.101.208:8090/botbuddies/paycafe', {store_seq : store.store_seq, user_id:user_id, orders:orderDetails, store_user:store.user_id})
        
      }else{
        const response = await axios.post('http://18.188.101.208:8090/botbuddies/payment', {store_seq : store.store_seq, user_id:user_id, orders:orderDetails, selectedTable:selectedTable, store_user:store.user_id})
      }
